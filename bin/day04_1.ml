...
let () =
  ...
  let seen_nums, winning_board, last_seen_num =
    List.fold_until numbers ~init:Int.Set.empty
      ~finish:(fun _ -> failwith "no winner")
      ~f:(fun seen_so_far num ->
        let with_num = Int.Set.add seen_so_far num in
        match List.find boards ~f:(Board.is_won ~seen_nums:with_num) with
        | None -> Continue_or_stop.Continue with_num
        | Some winner -> Continue_or_stop.Stop (with_num, winner, num))
  in
  print_endline
